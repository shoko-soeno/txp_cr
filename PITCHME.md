...
- RCT、コホート、横断研究　内的妥当性の評価のため主要変数(exposureなど)の分布を示す
- 症例対照研究では、疾患（症例 vs 対照）の状態で層別化するのが最も一般的（Figure 2, point 1)
    - 選択バイアスは評価できるが、症例と対照群では転帰の原因の分布が異なると予想されるため交絡は評価できない
    - 対照群をさらに被曝で層別化→潜在的な交絡因子の分布を比較することができる（Figure 2、point 2）
...
---
#### Considering rows
- 最終的な分析に含まれる主要な研究変数に加えて、他の潜在的な交絡因子を含めることで残留交絡や選択バイアスを評価
評価された変数（Figure 3、point ７）
選択変数（例：サンプリングで使用された変数、研究への参加やフォローアップへの喪失に影響を与える可能性のある変数）
選択バイアスの存在を示す可能性のある変数（例：病院ベースの症例対照研究における自宅から病院までの距離）（Figure 2、point 6）。
- 外部妥当性の評価
重要な潜在的な効果修飾因子を含む主要なベースライン人口統計の分布を示す行を追加することは、原集団と特定の対象集団との間の関心のある効果の有意差の可能性を評価する際に読者にとって有用である。
効果の修正がない場合であっても 明示的にモデル化された場合でも、可能な効果修飾子の分布を示すことは、外部妥当性の脅威を評価するのに有用（図２、ポイント８）。

- 時系列分析を含む研究　
曝露で層別化したperson-timeの要約
    1人当たりの総計
    平均または中央値（外的妥当性の評価に有益）
    打ち切りの理由（内的妥当性の評価に必要）

- 変数が収集された方法とは異なる方法で分析される場合（連続変数のカテゴリ化など）、一般的には主分析で分析された変数を含める。
- 測定誤差または残留交絡の評価のためと、外的妥当性評価のため、もとの連続変数を示すこともある（Figure 2, point 4、Figure 3, point 6）。

---
#### Considering cells
- 視覚的な乱雑さを減らすことに重点を置く
カテゴリカル変数は％のみを表示し、列のヘッダーにNを表示したり、％を整数に丸める
- 平均と標準偏差は一般的ですが、正規分布でない場合には最小・最大値、中央値、四分位を含む要約を示す（Figure 2、point 7）
- 分布に関する情報は，読者が 
a) 測定誤差があるかどうかを評価するのに役立ちます。がデータ収集で発生し、内部妥当性が損なわれた場合（例えば 血圧測定値が予想よりも低い）
b)影響力のあるデータポイントがあるかどうか は、報告された効果推定値に不当なレバレッジをかける可能性がある（また、潜在的には 内部妥当性）
c) 標本内の分布がどのようにターゲットと比較されるか 結果の外部妥当性を知らせる母集団。
